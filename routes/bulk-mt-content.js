...
router.post('/:product_name', function(req, res, next) {	
	...
	debug
	? logger.info('Request method ' + '(/v1/subscriber/bulk-mt-content)' + ' : ' + JSON.stringify(reqFull))
	: logger.info('Request method ' + '(/v1/subscriber/bulk-mt-content)' + ' : ' + JSON.stringify(reqFull));

	asyncResolveBulkMtContent(reqFull, (err, rs) => {
		var response = {};			
		if (!err){
			...
			debug
			? logger.info('Response method ' + '(/v1/subscriber/bulk-mt-content)' + ' : ' + JSON.stringify(response))
			: logger.info('Response method ' + '(/v1/subscriber/bulk-mt-content)' + ' : ' + JSON.stringify(response));
 ...
 */
 var asyncResolveBulkMtContent = (data, cb) => {
	...
	(data, cb) => {
		...
		debug
		? logger.debug('step' + data.step + ' : Persisto request en redis') 
		: null;
		...
		try{
			...
			k = "apifox-bulk-mt-content-" + dataFull.body.content_id;
