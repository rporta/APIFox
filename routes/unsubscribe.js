 ...
 */
 var asyncResolveUnsubscribe = (data, cb) => {
	...
	// (data, cb) => {
	// 	dev
	// 	? logger.debug('asyncResolveUnsubscribe(): Execute process... 2') 
	// 	: null;
	...
	// (data, cb) => {
	// 	dev
	// 	? logger.debug('asyncResolveUnsubscribe(): Execute process... 3') 
	// 	: null;
	...
	var final = (err, rs) => {
		dev
		? logger.debug('asyncResolveUnsubscribe(): Execute final... ')
		: null;

		err
		? (() => {
			logger.debug('asyncResolveUnsubscribe(): Error final... ');
			console.log(err);
			cb(err, rs);
		})()
		: cb(err, rs);
