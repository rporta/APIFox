...
var redisLogger = new(winston.Logger)({
	transports: [
		...
		new(winston.transports.File)({
			...
			filename: 'logs/mtprocess_redis_access.log',
			json: false
		})
	]
});
	...
	logger = new(winston.Logger)({
		transports: [
			...
			new(winston.transports.File)({
				...
				filename: 'logs/mtprocess_access.log',
				json: false
			})
		]
	});
...
var mssqlLogger = new(winston.Logger)({
	transports: [
		new(winston.transports.File)({
			...
			filename: 'logs/mtprocess_mssql_access.log',
			json: false
		})
	]
});
...
heartbeats.init(config.server.mtProcess.serviceName);
			...
			async.eachSeries(data.redisGetRequest, function(currentRequest, next) {
					...
					if (currentRequest.body.data_option_1) {
						...
						for (var f in currentRequest.body.data_option_1.user_ids) {
							...
							for (var i in productMap[currentRequest.path.product_name].paqueteid) {
								//preaparo params para la query 'opradb.isActive'
								var pia = {};
								pia.paqueteid = productMap[currentRequest.path.product_name].paqueteid[i];
						...
						var paramsBillingStatus = new Array();

						for (var i in productMap[currentRequest.path.product_name].paqueteid) {
							var pr = new Object();
							var current = productMap[currentRequest.path.product_name].paqueteid[i];
							pr.sponsorid = countryInfo[currentRequest.body.carrier_id].sponsorId;
							pr.paqueteid = current;
							//pr.interval = countryInfo[currentRequest.body.carrier_id].interval;
							//pr.top = processMtContent.top; // page_size --aca va 10.000 by Benja, se cambia a 100 poner en config
							paramsBillingStatus.push(pr);
						}


						// logger.debug('paramsBillingStatus : ' + JSON.stringify(paramsBillingStatus));
